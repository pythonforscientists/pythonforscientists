...
There are two steps to creating a new \indexterm{variable} in Python: declaration\index{variable!initialization} and initialization. \newterm[Declaration]{variable!declaration} means that you are telling Python that you are going to create a new \indexterm{variable}. \newterm[Initialization]{variable!initialization} is the process of putting some initial data into that \indexterm{variable} that can then be manipulated later. Initializing a \indexterm{variable} doesn't necessarily mean that you cannot change the \indexterm{variable}'s value later.\footnote{The exception to this is if you decide to use constant values, or consts. You might set a constant for the value of Pi or for the force of gravity. Constants cannot be changed after initialization.} Declaration\index{variable!declaration} and initialization\index{variable!initialization} are typically done on the same line of code. By using a single equal sign \verb|=|, we can initialize\index{variable!initialization} and declare a \indexterm{variable} in one fell swoop. Python will do its best to figure out what kind of \indexterm{variable} it is, depending on what kind of information you give it. Consider the following code.\par
\begin{lstlisting}[style=pippython]
cashValue = 3.39
...
kitties = 390
\end{lstlisting}
This code is declaring the four new variables \verb|cashValue|, \verb|accountHolder|, \verb|isBroke|, and \verb|kitties|. Each of these variables is being initialized\index{variable!initialization} to a different type. \verb|cashValue| is being initialized\index{variable!initialization} to the value \verb|3.39|, which would be stored as a float. Likewise, \verb|accountHolder| is being initialized\index{variable!initialization} to the value \verb|"Stinky Pete"|, which is a string. \verb|isBroke| is being initialized\index{variable!initialization} to the \indexterm{Boolean} value \verb|True|. Note that \verb|kitties| is being initialized\index{variable!initialization} to 390, and Python will try to store this value as an integer, since there isn't a decimal point. If we wanted to force Python to store \verb|kitties| as a float, we could just add a \verb|.0| to the end of the initialization\index{variable!initialization} value:\par
\begin{lstlisting}[style=pippython]
kitties = 390.0
\end{lstlisting}
When declaring or updating the value of a \indexterm{variable}, the name of the \indexterm{variable} always goes to the left side of the equal sign. You can think of a \indexterm{variable} statement as putting the thing on the right side of the equal sign into the \indexterm{variable} on the left side of the equal sign. So, the following code is syntactically correct:
\begin{lstlisting}[style=pippython]
accountHolder = "Stinky Pete"
\end{lstlisting}
...
\end{lstlisting}
In the above examples, can you guess what the variables are describing? Probably. The \indexterm{variable} names \verb|cashValue|, \verb|accountHolder|, and \verb|isBroke| explain what information the variables hold pretty well. However, the \indexterm{variable} \verb|kitties| doesn't really make sense. Is this how many kitties I have? Is it my love or hatred of cats on a scale from 1 to 1000? We don't really know. In programming (not just in Python), it is critically important that you get into the good habit of declaring good \indexterm{variable} names. Be descriptive, but concise. A better \indexterm{variable} name might be \verb|numKitties|.\par
...
\begin{itemize}
    \item A \indexterm{variable} name must start with a letter or the underscore character \verb|_|
    \item A \indexterm{variable} name cannot start with a number
    \item A \indexterm{variable} name can only contain alphanumeric characters and underscores (A-Z, a-z, 0-9, and \_)
    \item Variable\index{variable} names are case-sensitive: \verb|age|, \verb|Age|, and \verb|AGE| are three different variables
    \item Variable\index{variable} names cannot be reserved words in Python, such as \verb|print|
\end{itemize}
...
\begin{itemize}
    ...
    \item lowercaseCamelCase is written with the first word in all lowercase and all following words capitalized. In Python, lowercase camel case is typically used for \indexterm{variable} names.
...
It is possible to declare\index{variable!declaration} a new \indexterm{variable} without initializing\index{variable!initialization} it, but you must declare\index{variable!declaration} a \indexterm{variable} before initializing\index{variable!initialization} it. If you don't know what type of \indexterm{variable} will be, but you know that you'll need a \indexterm{variable}, you can initialize\index{variable!initialization} to something called the \newterm[Nonetype]{Nonetype}. To do so, simply set the initialization\index{variable!initialization} to \verb|None|. This is the practical equivalent to declaring\index{variable!initialization} without initializing\index{variable!initialization}.\footnote{Other languages have no Nonetype, since they support direct declaration\index{variable!initialization} without initialization. For example, in C++, you could run the line 'int age;', which would declare\index{variable!declaration} the 'age' \indexterm{variable} without initializing\index{variable!initialization} it with anything.} Consider the following code.\par
\begin{lstlisting}[style=pippython]
emptyVar = None
print(type(emptyVar))
\end{lstlisting}
...
<class 'NoneType'>
\end{lstlisting}
In general, you should always try to initialize\index{variable!initialization} your variables, even if it's just to \verb|0| (if it's an integer), \verb|0.0| (if it's a float), or \verb|""| (if it's a string). If it's a \indexterm{Boolean} \indexterm{variable}, you can initialize\index{variable!initialization} it to \verb|True| or \verb|False|, depending on what makes sense.\par
\boxtext{Initialize to type variables}{Initializing your variables helps you determine what datatype they should be, since Python is a weakly-typed language. As you become more experienced with Python datatypes, you should be able to instantly determine a datatype just by looking at the \indexterm{variable}'s first initialization\index{variable!initialization}. Therefore, it's worth learning the datatypes by heart - it'll come in handy later!}
We've talked about variables, but there's a specific type of \indexterm{variable} in Python called a constant.\footnote{Yes, in math, variables and constants are different concepts, but in Python, a constant is a subset of the \indexterm{variable}.} The difference between a regular \indexterm{variable} and a constant is that a constant \textit{cannot} be changed after initialization\index{variable!initialization}. To mark a constant, you should name it in all caps. It might be useful to use constants to store data that you know will never change, such as a conversion factor or a mathematical constant, like $\pi$ or $e$.\par
\begin{lstlisting}[style=pippython]
PI = 3.1415
E = 2.71
GRAVITY = 9.8
\end{lstlisting}
\warningtext{Changing Constants}{Some languages, like Python, treat constants and variables in the same way, meaning that it is possible to change the value of a constant (although you shouldn't!) Other languages essentially lock away the constant once it is declared\index{variable!declaration} as a constant. In these languages, after the first constant initialization\index{variable!initialization}, the interpreter or compiler will throw an error or warning if you try to reinitialize\index{variable!initialization} a constant.}
\subsubsection*{Exercise Questions}
These exercise questions cover chapter 4.2. 
\begin{Exercise}
    \Question{When you create a \indexterm{variable}, where is it stored: primary, secondary, or tertiary memory?}
    \Question{What are the two steps for creating a \indexterm{variable}?}
    \Question{Can you do those two steps in one line, or must they be done in two lines?}
    \Question{What character should you use to initialize\index{variable!initialization} a \indexterm{variable}?}
\end{Exercise}
\begin{Exercise}
    \Question{What is the difference between a datatype and a \indexterm{variable}?}
    \Question{What datatype can a \indexterm{variable} be?}
\end{Exercise}
\begin{Exercise}
    What is a good \indexterm{variable} name for the following pieces of data? What type of \indexterm{variable} would Python probably initialize\index{variable!initialization} it to? Provide an example of how Python would store that data. For example, if you came up with a \indexterm{variable} named \verb|numCookies| that was an integer, then an example of the data might be \verb|5|.
...
\end{Exercise}
\subsection{Referring to Variables\index{variable}\index{variable}}
...
The way in which you refer to an existing \indexterm{variable} will depend on the manner in which you need to refer to it. In most cases, it is possible to simply refer to a \indexterm{variable} by name. For example, let's declare a new \indexterm{variable} as a string, then print the contents of that \indexterm{variable}.
\begin{lstlisting}[style=pippython]
...
Python is cool!
\end{lstlisting}
Observe how we are \textit{not} using any quotation marks inside of our print statement. This is because we are not actually printing "stringToPrint," but rather, we are printing the string literal that is stored as a value of the \indexterm{variable} \verb|stringToPrint|.\par
Recall how we created a \indexterm{variable} and filled with something. The process of creating the \indexterm{variable} was called the declaration\index{variable!initialization}, and the "filling" was called the initialization\index{variable!initialization}. We can also reinitialize\index{variable!initialization} a \indexterm{variable} by simply overwriting the contents of the memory space that the \indexterm{variable} uses. We need not redeclare\index{variable!initialization} the \indexterm{variable}, since it already exists. Attempting to redeclare\index{variable!initialization} an existing \indexterm{variable} will result in a syntax error. Consider the following code.\par
\begin{lstlisting}[style=pippython]
...
Python is rad!
\end{lstlisting}
In this code sample, we are printing the same \indexterm{variable} \verb|string1| twice. However, we also observe that the output changes. This is because we're reinitializing the \indexterm{variable} in the third line using \verb|string1 = "Python is rad!"|. Because of this, we will get a different result when we try to print the same \indexterm{variable}.\par
...
7
\end{lstlisting}
Since we're changing the contents of the \indexterm{variable}, the second print statement is simply getting the updated \indexterm{variable} value.\footnote{For now, ignore the str() function. This is typecasting the integer into a string so that it can be printed by the print() function. We'll cover typecasting in chapter 4.6.}\par
\subsubsection*{Exercise Questions}
These exercise questions cover chapter 4.2.2.
\begin{Exercise}
    \Question{Define a new \indexterm{variable} \verb|name| and initialize it to your name.}
    \Question{Now, print "Your name is: " and concatenate it with the \verb|name| \indexterm{variable} in the print statement.}
\end{Exercise}
\begin{Exercise}
    \Question{Using your \indexterm{variable} naming rules, create a new \indexterm{variable} that would describe the name of a building on your college campus. Initialize it to the name of a building on your college campus.}
    \Question{Create a new \indexterm{variable} that describes the discipline of that building (math, history, psychology, English, etc.) and initialize it to that value.}
    \Question{Write a print statement that will print "\_\_\_ is a \_\_\_ building." Fill the first blank with the \indexterm{variable} for your building's name, and fill the second blank with the \indexterm{variable} for the discipline of that building. Use string concatenation to print all of the variables from the same print statement.}
\end{Exercise}
...
On the console, the function to take in user input is quite aptly named \verb|input()|. \verb|input()| must always be preceded by a \indexterm{variable} and the \indexterm{variable} assignment operator; it cannot stand on its own like \verb|print()| can. Consider the following example code.\par
\begin{lstlisting}[style=pippython]
...
\end{lstlisting}
When you run this code, your program will prompt you for your name. You can then type directly into the console using your keyboard. When you press Enter, the contents that you typed are put into the \indexterm{variable} \verb|name| as a string. The last line then prints some string literals and your name.\par
When using \verb|input()|, it doesn't matter whether you input only letters or only numbers. The \verb|input()| function will always return the input as a string. You can, however, typecast a \indexterm{variable} as some other datatype. See chapter 4.6 on typecasting.\par
The \verb|input()| function can take one argument, a string to prompt the user. You can either pass in a \indexterm{variable} which is of type string or just use a string literal. So, the following code is logically the same as the previous example.\par
\begin{lstlisting}[style=pippython]
...
\begin{Exercise}
    \Question{Is it possible to use the \verb|input()| function standalone (that is, without an \indexterm{variable} and assignment operator)? Why or why not?}
...
\end{Exercise}
\begin{Exercise}
    \Question{Create a new \indexterm{variable} \verb|age| and initialize it to the Nonetype. See chapter 4.2.1 for a review on the Nonetype.}
    \Question{Use the \verb|input()| function to ask for the user's age and put the result into the \verb|age|.}
    \Question{Print the datatype of \verb|age|. What datatype is it? Why is this so? (Hint: to print the datatype of a \indexterm{variable}, you can print the \verb|type(variableName)|.)}
\end{Exercise}
\begin{Exercise}
    \Question{Create a new \indexterm{variable} \verb|color| and initialize it to be an empty string.}
    \Question{Write an \verb|input()| statement that prompts the user for their favorite color and puts the result in the \verb|color| \indexterm{variable}.}
    \Question{Write a single print statement that prints the user's favorite color and compliments their color choice.}
    \Question{Write another \verb|input| statement that prompts the user for their least favorite color and overwrites the contents of the \verb|color| \indexterm{variable}.}
...
\section{Whitespace\index{whitespace}}
Whitespace\index{whitespace} is a crucial component of every programming language, and using \indexterm{whitespace} correctly is very important. Different languages handle \indexterm{whitespace} differently, but you should know how to correctly format your code, regardless of which language you're writing code in.\par
\subsection{Why is Whitespace\index{whitespace} Important?}
Donald Knuth once said, "Programs are meant to be read by humans and only incidentally for computers to execute." As programmers, it is in your best interest to make your code readable. More readable code means that you can understand it, find problems faster, and others can catch onto your programming style quickly and understand your code swiftly. Messy code is difficult to read. Even if your instructor does not require you to or does not penalize for incorrect \indexterm{whitespace}, it is still an important skill.\par
...
Consider the following code, which uses poor or inconsistent \indexterm{whitespace}.\par
\begin{lstlisting}[style=pippython]
if(a>0):
  print(a)
else:
...
if (a > 0):
  ...
  if (a < 10):
    ...
    print("a is negative or greater than 10")
\end{lstlisting}
Observe how this code uses consistent \indexterm{whitespace} patterns for all of its indentation. One indentation is, in this case, always two spaces. Indentation doesn't always need to be two spaces. It could just as easily be four or eight spaces, or a tab character.\par
Regardless of which \indexterm{whitespace} rules you choose to follow and those you choose to ignore, think about how your decisions will affect the readability of the code. \par
Whitespace\index{whitespace} is also vertical. You can generally put empty lines at logical breaks in the code, such as between loops, functions, or if/else statements. Consider the following code.\par
\begin{lstlisting}[style=pippython]
...
def function3(arg1, arg2):
  return arg1 // arg2
\end{lstlisting}
You don't need to understand exactly what this code does, but while it does have good horizontal \indexterm{whitespace}, we could improve its vertical \indexterm{whitespace} by adding extra empty lines. Compare the previous example to the following code.\par
\begin{lstlisting}[style=pippython]
def function1(arg1, arg2):
  return arg1 + arg2
...
\subsection{Common Whitespace\index{whitespace} Patterns}
There are several \indexterm{whitespace} patterns that are used in programming, especially in Python. In general, follow the following rules.\par
\begin{itemize}
...
You can play around with which \indexterm{whitespace} rules you'd like to use and which you will ignore. The important thing is that you try to stay organized and that you are consistent throughout your entire source code file or project.\par
\subsubsection*{Exercise Questions}
These exercise questions cover chapters 4.3.1 and 4.3.2.
\begin{Exercise}
    \Question{What is \indexterm{whitespace}?}
    \Question{What directions does \indexterm{whitespace} apply: horizontal, vertical, or horizontal \textit{and} vertical?}
    \Question{Why is \indexterm{whitespace} so important?}
    \Question{Does your course instructor or grader penalize for improper \indexterm{whitespace}? If so, what is the penalty?}
\end{Exercise}
\begin{Exercise}
...
\end{Exercise}
\begin{Exercise}
    \Question{What should surround the assignment operator to achieve good \indexterm{whitespace}?}
    \Question{What should surround a comparison operator for good \indexterm{whitespace}?}
    \Question{Should you put an extra space in between two characters in a comparison operator (\verb|!=| versus \verb|! =|)?}
    \Question{Is it okay to change the indentation standard or \indexterm{whitespace} pattern in the middle of a project? When is it okay to, if ever?}
...
\end{Exercise}
\begin{Exercise}
Fix the following lines of code by correcting the \indexterm{whitespace} errors. What did you fix, and why did you choose to make that fix? If there are no errors, say so.
\Question{
\begin{lstlisting}[style=pippython]
if(var!=3):
\end{lstlisting}
...
\end{Exercise}
\section{Comments}
Comments are a critical part of any program, even if they don't contribute to the actual functionality of your code. Just like how \indexterm{whitespace} is so important, your comments are the key for you being able to understand your code. Talk to any experienced programmer, and they'll have stories of how they spent all night working on a chunk of code, then forgot what it meant or how it worked the very next day and had to rewrite the entire chunk of code.\par
...
\end{lstlisting}
Block comments are a powerful tool, and they can be used to test parts of your code without having to write pound signs before every single line. For example, consider the following code.\footnote{It's worth noting, however, that some IDEs support the ability to comment multiple lines at once through a keyboard shortcut or menu option. When you choose an option to comment multiple lines in the menu, the IDE will almost never make a block comment, instead inserting pound signs before every line. The same goes for uncommenting those lines - the IDE will remove the pound signs before the line. However, not all IDEs support this behavior. When you are sharing code with others who might not be using the same IDE as you, you should be very careful to use this feature, since your compatriot might have to manually remove those same pound signs. This means that they could end up having to manually remove hundreds of pound signs if you had mass-commented out hundreds of lines using your IDEs multiline comment feature.}\par
\begin{lstlisting}[style=pippython]
...
\end{lstlisting}
Also note that some IDEs and text editors might color your block comments differently from your line comments. This is nothing to be worried about.\footnote{The reason for this is because a block comment in Python is actually a multiline string. Suppose you had a really big string that included lots of horizontal breaks (new lines, carriage returns). Writing your string as a multiline comment allows you to skip the \textbackslash n and \textbackslash r marks. However, multiline strings make excellent block comments, and this has become the defacto way of making a block comment in Python.}
\subsubsection*{Exercise Questions}
...
\begin{itemize}
    ...
    \item \verb|NameError|: raised when a locall or global name (\indexterm{variable}) is not found
...
\end{lstlisting}
You don't have to understand exactly what this code does, but do pay attention to the exception. This code tests if the \indexterm{variable} \verb|x| is an integer. If it isn't, then it'll throw a generic exception. We can also throw a \verb|TypeError|, since we could generally classify this as a datatype error. Consider the following code.\par
\begin{lstlisting}[style=pippython]
x = "hello"
if not type(x) is int:
...
A little bit of sleuthing in the Python documentation reveals that \verb|input()| always returns a string. Consider the following code. Assume that the \indexterm{variable} \verb|price| has not been declared\index{variable!initialization} or initialized already.
\begin{lstlisting}[style=pippython]
price = input("Input the total price of your groceries as a decimal:")
print(price)
\end{lstlisting}
What datatype would \verb|price| be? No matter what kind of data we try to input in our program, \verb|price| will always be a string, since that's the datatype that \verb|input()| will always insert into the \indexterm{variable} that it is being assigned to.\par
...
\end{lstlisting}
This program expands on our previous program by calculating our tax. It multiplies the float \indexterm{variable} \verb|total| by a given tax rate, 8\%. However, I'd like to draw your attention to the second line:\par
\begin{lstlisting}[style=pippython]
price = float(price)
\end{lstlisting}
We can see that we're using a previously declared\index{variable!initialization} and initialized \indexterm{variable} \verb|price| and we're reinitializing the \indexterm{variable}'s value to the typecasted value of the original \indexterm{variable}.\par
In line 1, \verb|price| was declared\index{variable!initialization} a string, since it was assigned a string by the \verb|input()| function.\par Then, in line 2, we typecast that string into a float and replaced the value of the original string with the new float. In line 3, we did some arithmetic on our float and put the result into the \indexterm{variable} \verb|tax|. We don't need to typecast \verb|tax|, since the value of \verb|price * 0.08| is already a float. Finally, in line 4, we're printing a string literal using string concatenation.\par
...
\end{tabular}
\vspace{5mm}\par
In the above table, the argument can be any type of \indexterm{variable}: a Boolean, integer, float, or string. However, it's worth noting that while you \textit{could} pass any datatype into your typecasting function, not all \verb|variables| can be passed in. For example, consider the following example.
\begin{lstlisting}[style=pippython]
...
Again, assume that \verb|price| and \verb|tax| are undeclared\index{variable!initialization} and uninitialized.\par
\begin{lstlisting}[style=pippython]
price = 0.0
while price == 0.0:
  try:
...
print("You need an extra \$", tax, " for tax, sep = "")
\end{lstlisting}
We'll get to what the \verb|while| is in chapter 6.3, but the gist of the code is, Python will declare\index{variable!initialization} a new \indexterm{variable} \verb|price| to be 0.0. As long as the price is 0.0, Python will assume that the user hasn't inputted a valid price and will keep prompting the user until they enter a valid price that can be typecast into a float. Then, it can carry on with the rest of the program.\par
\subsubsection*{Exercise Questions}
...
\end{Exercise}
\begin{Exercise}
Write the applicable code to typecast the following variables into a string. If typecasting cannot be performed or is not applicable to the \indexterm{variable}, explain why.
    \Question{\verb|v = 3|}
...
\end{Exercise}
\begin{Exercise}
Write the applicable code to typecast the following variables into an integer. If typecasting cannot be performed or is not applicable to the \indexterm{variable}, explain why.
    \Question{\verb|v = 3|}
...
The cat is orange.
\end{lstlisting}
Traditionally, if we wanted to print the \indexterm{variable} \verb|color|, we would need to do this.
\begin{lstlisting}[style=pippython]
color = "orange"
...
The cat is orange.
\end{lstlisting}
This should look familiar. We still have a string literal portion ("The cat is "), a \indexterm{variable} ("orange"), and another string literal ("."). However, we could use a f-string to avoid having to close our quotes out at all. To do this, we can simply enclose our \indexterm{variable} inside of curly braces \verb|{}|. The curly braces tell Python that the contents of the curly brace are actually a \indexterm{variable}, and we want Python to take that \indexterm{variable} instead of the literal string.\par
...
In the above f-string, take note of the following. The f-string is predicated with a \verb|f|. This tells Python that we want to use a f-string, rather than a traditional string. Next, there is no space between the \verb|f| and the opening quote \verb|'|. Putting a space between the \verb|f| and the opening quote \verb|'| is not syntactically correct. Finally, our \indexterm{variable} \verb|color| is only enclosed in curly braces inside of our f-string.\par
If we had a non-string value in a \indexterm{variable}, like an integer or float, we cannot just print that value. As a review, we end up with a \verb|TypeError|.
\begin{lstlisting}[style=pippython]
...
The value is 1
\end{lstlisting}
In a f-string, we do not need to typecast a non-string \indexterm{variable}. Typecasting is implicitly done at runtime to any non-string values, meaning that we can simply call the object.\par
\begin{lstlisting}[style=pippython]
value = 1
print(type(value))
...
F-strings are a great way to increase the readability of your code. They decrease the amount of extra symbols, since we don't need to explicitly concatenate, typecast, or close our string to still call a \indexterm{variable} inside of a string.
\subsubsection*{Exercise Questions}
These exercise questions cover chapter 4.7.
\begin{Exercise}
\Question{What is a f-string, and how is a f-string different from a standard string?}
\Question{What special characters are used in a f-string?}
\Question{How do we tell Python that we want to use a f-string?}
\Question{Is typecasting required when using a f-string?}
\end{Exercise}
\begin{Exercise}
\Question{In a new Python script, write a \verb|input()| statement that asks the user for their name and puts the return into a new variable called \verb|name|.}
\Question{What is the datatype of \verb|name|?}
\Question{Print \verb|Your name is| and the value of \verb|name| using a standard string with concatenation (not using a f-string).}
\Question{Print \verb|Your name is| and the value of \verb|name| using a f-string.}
\end{Exercise}
\begin{Exercise}
\Question{In a new Python script, create a \indexterm{variable} named \verb|value| and initialize the \indexterm{variable} to \verb|10.60|.}
\Question{Print \verb|The value is| and the value of \verb|value| using a standard string with concatenation (not using a f-string).}
\Question{Print \verb|The value is| and the value of \verb|value| using a f-string.}
\end{Exercise}
\begin{Exercise}
\Question{In a new Python script, create two variables named \verb|weight| and \verb|height|. Set \verb|weight| to be a float and \verb|height| to be an integer.}
\Question{Initialize the value of \verb|weight| to be a weight in kilograms.}
\Question{Initialize the value of \verb|height| to be a height in centimeters.}
\Question{Print \verb|The weight is|, the value of \verb|weight|, \verb|, and the height is|, and the value of \verb|height|. Use a standard string with concatenation (not using a f-string).}
\Question{Print \verb|The weight is|, the value of \verb|weight|, \verb|, and the height is|, and the value of \verb|height|. This time, use a f-string.}
\end{Exercise}
\begin{Exercise}
\Question{When using string concatenation in a \verb|print()| statement, what is the default separator character?}
\Question{When using a f-string, is there a default separator character from the string literal to the variable? Provide an example to prove your point.}
\end{Exercise}
\section{Statements and Expressions Review}
So far, we've learnt of several different statements and expressions. From \indexterm{variable} declaration\index{variable!initialization} and initialization to function calls, you've already learned how to use statements. Let's look at some other types of statements that you'll run into, especially as your programming skills grow.\par
Firstly, let's review the assignment operator \verb|=|. The assignment operator is used to assign a value to a \indexterm{variable}. For example, consider the following code.\par
\begin{lstlisting}[style=pippython]
cats = "tabby cat"
\end{lstlisting}
...
\end{lstlisting}
However, if the function doesn't produce something, then Python will create a syntax error. The only way to prevent this from happening is to handle the exception; using exception handling, you can assign the Nonetype to the \indexterm{variable}. For example, consider the following code.\par
\begin{lstlisting}[style=pippython]
...
Lastly, we've seen the block format, where certain code routines are run as part of another block of code. Most recently, we've seen this used in try...except blocks. To denote that a code routine should run as a part of another line of code, like \verb|try| or \verb|except|, we should use an indentation. The exact style of indentation isn't terribly important, so you could use two spaces, four spaces, a tab, or two tabs, but according to our \indexterm{whitespace} rules, it's important to be consistent with whichever style we choose to use.\par
...
Firstly, let's discuss where you might use these symbols. In programming (not just in Python), \indexterm{arithmetic} operations are typically run next to an assignment operator \verb|=|, so that their result can be assigned to a \indexterm{variable}. Consider the following code.
\begin{lstlisting}[style=pippython]
seven = 2 + 5
\end{lstlisting}
Observe how the \indexterm{variable} name is on the left side and the value is on the right side. Recall how the assignment operator must always be used in this manner, with the \indexterm{variable} name on the left and the value to be assigned to that \indexterm{variable} on the right.\par
...
x = 2 + 4
$$
This is a mathematically correct equation. We're taking some value on the right side and putting it into the \indexterm{variable} $x$. Now, consider the following in a mathematical context:\par
$$
x = x + 2
$$
...
x = x + 2
\end{lstlisting}
In this case, we're initializing the value of the \indexterm{variable} \verb|x| to be 5, then we're adding 2 to the value of \verb|x|. Even though this is not \textit{mathematically} correct, this is correct in \textit{programming}. However, it's worth noting that the following is \textit{not} syntactically correct:\par
\begin{lstlisting}[style=pippython]
x = 5 
x + 2 = x # WRONG
\end{lstlisting}
As we saw in chapter 4.2, our \indexterm{variable} needs to be the only thing on the left side. Again, this might be mathematically correct, but it's incorrect in Python.\par
\begin{lstlisting}[style=pippython]
2 + 4 = x # WRONG
\end{lstlisting}
is not the same as 
...
\end{Exercise}
\begin{Exercise}
Write the applicable code to evaluate the following lines in Python. Imagine that you are typing directly into the Python interpreter as a calculator. That means there's no need to put the result into a \indexterm{variable}.
